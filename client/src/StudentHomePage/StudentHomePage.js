...
class StudentHomePage extends React.Component {
  ...
  componentDidMount() {
    axios({
      ...
      this.state.studentInfo.forEach((i) => this.getTbl(i.class_id));
    }).catch((error) => {
    ...
    if (this.state.newStudent) {
      return (
        <div className="greeting">
          <h2><Link to={{ pathname: '/student/settings/' }}>Click Here To Register For a Class!</Link></h2>
          <br />
      ...
      this.table = this.state.assignments.map((data, i) => (
        <tr>
          <td>
            ...
            <br />
            <i>{new Date(data.assigned_date).toLocaleString()}</i>
          </td>
  ...
  render() {
    return (
      <div>
        ...
        <Link to="/student/home/classes">
          {' '}
          <input type="submit" value="View Registered Classes" />
          {' '}
        </Link>
      </div>
    );
