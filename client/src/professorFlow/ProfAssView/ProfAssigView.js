...
class ProfAssigView extends React.Component {
  constructor(props) {
    ...
    const rId = this.props.match.params.rubricId;
    ...
    axios({
      ...
      headers: {
        ...
        const e = res.data[i];
        const q1 = { value: e.question_text, min_char: e.min_char };
        const q = {};
        q[e.question_id] = q1;
  ...
  Question(pId, qId) {
    const question = this.state.prompts[pId].questions[qId];
    return (
      <div key={`q${qId}`}>
        <h3>
          {' '}
  ...
  Prompt(pId) {
    return (
      <div key={`p${pId}`}>
        <h2>
          Prompt:
          {this.state.prompts[pId].value}
        </h2>
        {
					Object.keys(this.state.prompts[pId].questions).map((qId) => this.Question(pId, qId))
				}
      </div>
    );
  }
  ...
  Assignment() {
    return (
      <div>
        {
				Object.keys(this.state.prompts).map((pId) => this.Prompt(pId))
			}
      </div>
    );
  }
